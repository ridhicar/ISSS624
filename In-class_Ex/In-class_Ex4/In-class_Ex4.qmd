...
```{r}
mpsz = st_read(dsn = "/Users/ridz/ridhicar/ISSS624/Hands-On_Ex/Hands-on_Ex4/data/data/geospatial", layer = "MP14_SUBZONE_WEB_PL")
```
...
```{r}
condo_resale = read_csv("/Users/ridz/ridhicar/ISSS624/Hands-On_Ex/Hands-on_Ex4/data/data/aspatial/Condo_resale_2015.csv")
```
...
In this section, we draw small multiple histograms (also known as trellis plot) by using `ggarrange()` of [**ggpubr**](https://cran.r-project.org/web/packages/ggpubr/) package.
...
In this section, we will learn how to build hedonic pricing models for condominium resale units using [`lm()`](https://www.rdocumentation.org/packages/stats/versions/3.5.2/topics/lm) of R base.
...
If one prefers formal statistical test methods, the [`ols_test_normality()`](https://olsrr.rsquaredacademy.com/reference/ols_test_normality.html) of **olsrr** package can be used as shown in the code chunk below.
...
In this section, we are going to model hedonic pricing using both the fixed and adaptive bandwidth schemes.
...
### 6.9.6 Visualizing co-efficient estimates

The code chunk below is used to create an interactive point symbol maps side by side for easier comparison.
